							...
							<a onclick="framshow(${newmovie.movie_id})" class="atag">
								<div class="atagdiv">
									...
									${AllDirector.director_name }
													...
													<tr align="left">
														<td><span class="c-gay">출연: </span> <c:forEach
																...
																<c:if test="${newmovie.actor_id1 eq AllActor.actor_id }">
									...
									${AllActor.actor_name }<c:if
																...
																<c:if test="${newmovie.actor_id3 eq AllActor.actor_id }">
									${AllActor.actor_name }
									</c:if>
															</c:forEach></td>
													</tr>
													<tr align="left">
														<td><span class="c-gay">개요: </span>${newmovie.country }영화
															• <c:forEach items="${AllGenre }" var="AllGenre"
																varStatus="status">
																<c:if test="${newmovie.genre_id1 eq AllGenre.genre_id }">
																			${AllGenre.genre_name}</c:if>
																<c:if test="${newmovie.genre_id2 eq AllGenre.genre_id }">
																				${AllGenre.genre_name } </c:if>
															</c:forEach></td>
													</tr>
												</table>
